...
const Button = ({ 
  ...
  const baseStyles = 'inline-flex items-center justify-center font-semibold rounded-md transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 whitespace-nowrap'
  ...
  // Mobile-first sizes: larger on mobile for better touch targets
  const sizes = {
    sm: 'px-3 py-2.5 sm:py-2 text-sm min-h-[40px] sm:min-h-[32px]',
    md: 'px-4 py-3 sm:py-2.5 text-base sm:text-sm min-h-[44px] sm:min-h-[36px]', // 44px touch target on mobile
    lg: 'px-6 py-4 sm:py-3 text-lg sm:text-base min-h-[48px] sm:min-h-[40px]'
  }
